...
Future<Uint8List?> cropImageDataWithDartLibrary(
  ...
  Animation? src;
  //LoadBalancer lb;
  final DateTime timeDecode1 = DateTime.now();
  if (kIsWeb) {
    src = decodeAnimation(data);
  } else {
    src = await compute(decodeAnimation, data);
  }
  final DateTime timeDecode2 = DateTime.now();
  print('${timeDecode2.difference(timeDecode1)} : decode');
  if (src != null) {
    //handle every frame.
    src.frames = src.frames.map((Image image) {
      ...
      image = copyResizeCropSquare(image, 200);
      return image;
    }).toList();
  }
